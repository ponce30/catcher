...
1. `POP2B (SEC)` がマイナスまたは欠損の行は **エラーとして除外**
2. Player単位でグループ化
3. 各項目の **トップタイム** に集約 (POP2B/EXCは最小値、ARMは最大値、Teamは最初の非空)
4. POP2B(最速)昇順で `Rk` を1始まりで付与

prefix一致のおかげで `POP2B`/`POP2B (SEC)`/`POP2B (min)` どの形式でも動作する。新しい列を追加したい時は `COLUMN_MAP` に entry を足し `agg` を指定。
